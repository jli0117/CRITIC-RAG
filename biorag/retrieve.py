...
def pubmed_index_create(pubmed_embeddings_dir, start_index, pubmed_group_num):
    ...
    for i in tqdm(range(start_index, min(max_load_num, start_index+pubmed_group_num)), desc="pubmed load and add", dynamic_ncols=True):
        embeds_chunk_path = f"{pubmed_embeddings_dir}/PubMed_Embeds_{i}.npy"
        #pdb.set_trace()
        embeds_chunk = np.load(embeds_chunk_path, mmap_mode='r')
...
def pubmed_decode(pubmed_I_array, pubmed_articles_dir, pubmed_group_num):
    ...
    def combine_articles(pubmed_articles_dir, start_index, pubmed_group_num):
        pubmed_articles = []
        for i in tqdm(range(start_index, min(max_load_num, start_index+pubmed_group_num)), desc="articles load and add", dynamic_ncols=True):
    ...
    #pubmed_I_array = np.load(idx_array_savepath)
        ...
        for ith, indices in tqdm(enumerate(pubmed_I_array[start_index//per_load_num]), desc="decode and add", dynamic_ncols=True):
